...
impl InputEvent {
    ...
    pub fn decode(msg: &[u8]) -> Option<Self> {
        ...
        match (msg.first()?, msg.len()) {
            (0x01, 5) => Some(Self::PointerMotion {
                x: u16_at(1),
                y: u16_at(3),
            }),
            ...
            (0x03, 5) => Some(Self::PointerAxis {
                dx: u16_at(1) as i16,
                dy: u16_at(3) as i16,
            }),
            (0x04, 4) => Some(Self::Key {
                code: u16_at(1),
                pressed: wire_bool(msg[3])?,
            }),
            _ => None,
        }
    }
    ...
    pub fn encode(&self) -> Vec<u8> {
        match *self {
            Self::PointerMotion { x, y } => {
                [&[0x01][..], &x.to_le_bytes(), &y.to_le_bytes()].concat()
            }
            Self::PointerButton { button, pressed } => vec![0x02, button.to_wire(), pressed as u8],
            Self::PointerAxis { dx, dy } => {
                [&[0x03][..], &dx.to_le_bytes(), &dy.to_le_bytes()].concat()
            }
            Self::Key { code, pressed } => {
                [&[0x04][..], &code.to_le_bytes(), &[pressed as u8]].concat()
            }
        }
    }
}
...
mod tests {
    ...
    fn header_bytes_and_limit() {
        assert_eq!(
            encode_header(1920, 1080),
            Some([0xFF, 0xFF, 0x80, 0x07, 0x38, 0x04])
        );
        assert_eq!(
            encode_header(65535, 1),
            Some([0xFF, 0xFF, 0xFF, 0xFF, 0x01, 0x00])
        );
    ...
    fn seq_packet_bytes() {
        assert_eq!(
            encode_seq(0x0102_0304, 7),
            [0xFE, 0xFF, 0x04, 0x03, 0x02, 0x01, 7, 0, 0, 0]
        );
    }
    ...
    fn mouse_button_evdev_codes() {
        use MouseButton::*;
        let codes: Vec<u32> = [Left, Right, Middle, Back, Forward]
            .map(MouseButton::evdev_code)
            .into();
